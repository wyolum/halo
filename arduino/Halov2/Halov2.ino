...
void amp_profile(){
  ...
  for(i = 0; i < NUM_LEDS; i++){ // bright in the middle fade to sides
    ...
    leds[maphalo2(i)] = CHSV(hsv.hue, hsv.saturation, 127 + 128 * cos((i - NUM_LEDS/2) * 2 * PI / (.7 * NUM_LEDS)));
  }
}
...
void mona(){
  ...
  for(int i = 8; i < 16; i++){
    leds[maphalo2(i)] = CRGB::Red;
  }
  for(int i = 16; i < 24; i++){
    leds[maphalo2(i)] = CRGB::Green/8;
  }
}
void blue(){
  for(int i = 0; i < NUM_LEDS; i++){
    leds[maphalo2(i)] = CRGB::Blue;
  }
}
void red(){
  for(int i = 0; i < NUM_LEDS; i++){
...
void white(){
  for(int i = 0; i < NUM_LEDS; i++){
    leds[maphalo2(i)] = CRGB::White;
  }
}
void green(){
  for(int i = 0; i < NUM_LEDS; i++){
    leds[maphalo2(i)] = CRGB::Green;
  }
}
void rainbow() 
{
...
void bpm()
{
  ...
  for( int i = 0; i < NUM_LEDS; i++) { //9948
    leds[maphalo2(i)] = ColorFromPalette(palette, gHue+(i*2), beat-gHue+(i*10));
  }
}
